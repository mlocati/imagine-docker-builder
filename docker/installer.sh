...
installGit() {
    ...
    installAptPackages '^libcurl[0-9]*-gnutls$ ^libexpat[0-9]*$ gettext' 'libssl-dev ^libcurl[0-9]*-gnutls-dev$ ^libexpat[0-9]*-dev$'
    printf 'Downloading git v%s... ' "$1"
    installGit_dir="$(mktemp -d)"
    curl -sSLf -o - https://codeload.github.com/git/git/tar.gz/refs/tags/v$1 | tar xzm -C "$installGit_dir"
    printf 'done.\n'
...
installLibaom() {
    ...
    curl -sSLf -o - https://aomedia.googlesource.com/aom/+archive/v$1.tar.gz | tar xzm -C "$installLibaom_dir"
    printf 'done.\n'
...
installLibdav1d() {
    ...
    curl -sSLf -o - https://code.videolan.org/videolan/dav1d/-/archive/$1/dav1d-$1.tar.gz | tar xzm -C "$installLibdav1d_dir"
    printf 'done.\n'
...
installLibyuv() {
	...
	curl -sSLf -o - https://chromium.googlesource.com/libyuv/libyuv/+archive/refs/heads/main.tar.gz | tar xzm -C "$installLibyuv_dir"
    printf 'done.\n'
...
installLibavif() {
    ...
    curl -sSLf -o - https://codeload.github.com/AOMediaCodec/libavif/tar.gz/refs/tags/v$1 | tar xzm -C "$installLibavif_dir"
    printf 'done.\n'
...
installLibde265() {
    ...
    curl -sSLf -o - https://github.com/strukturag/libde265/releases/download/v$1/libde265-$1.tar.gz | tar xzm -C "$installLibde265_dir"
    printf 'done.\n'
...
installLibheif() {
    ...
    curl -sSLf -o - https://github.com/strukturag/libheif/releases/download/v$1/libheif-$1.tar.gz | tar xzm -C "$installLibheif_dir"
    printf 'done.\n'
...
installGraphicsmagick() {
    installAptPackages \
        '^libz[0-9\-]*$ ^libjpeg[0-9]*-turbo ^libpng[0-9\-]*$ ^libjbig[0-9\-]*$ ^libtiff[0-9]*$ ^libwebp[0-9]*$ ^libwebpdemux[0-9]*$ ^libwebpmux[0-9]*$ libxml2 ^liblcms2[0-9\-]*$ ^libfreetype[0-9]*$ ^libzstd[0-9]*$' \
        'libbz2-dev ^libjpeg[0-9]*-turbo-dev libpng-dev libjbig-dev libtiff-dev libwebp-dev libxml2-dev liblcms2-dev ^libfreetype[0-9]*-dev$ ^libzstd[0-9]*-dev$'
    printf 'Downloading GraphicsMagick v%s... ' "$1"
    installGraphicsmagick_dir="$(mktemp -d)"
    curl -sSLf -o - http://ftp.icm.edu.pl/pub/unix/graphics/GraphicsMagick/${1%.*}/GraphicsMagick-$1.tar.gz | tar xzm -C "$installGraphicsmagick_dir"
    printf 'done.\n'
...
installImagemagick() {
    installAptPackages \
        '^libz[0-9\-]*$ ^libjpeg[0-9]*-turbo ^libpng[0-9\-]*$ ^libjbig[0-9\-]*$ ^libtiff[0-9]*$ ^libwebp[0-9]*$ ^libwebpdemux[0-9]*$ ^libwebpmux[0-9]*$ libxml2 ^liblcms2[0-9\-]*$ ^libfreetype[0-9]*$ ^libzstd[0-9]*$ ^libopenjp2[0-9\-]*$ ^libdjvulibre[0-9]*$ ^libwmf[0-9\.\-]*$ ^libfontconfig[0-9]*$ ^libzip[0-9]*$' \
        'libbz2-dev ^libjpeg[0-9]*-turbo-dev libpng-dev libjbig-dev libtiff-dev libwebp-dev libxml2-dev liblcms2-dev ^libfreetype[0-9]*-dev$ ^libzstd[0-9]*-dev$ ^libopenjp2[0-9\-]*-dev$ libdjvulibre-dev libwmf-dev libfontconfig-dev libzip-dev'
    return
    installImagemagick_dir="$(mktemp -d)"
    printf 'Downloading ImageMagick v%s... ' "$1"
    curl -sSLf -o - https://www.imagemagick.org/download/releases/ImageMagick-$1.tar.xz | tar xJm -C "$installImagemagick_dir"
    printf 'done.\n'
